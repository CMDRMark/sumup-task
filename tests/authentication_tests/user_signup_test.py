...
from test_data.invalid_data.invalid_user_auth_data import invalid_signup_data
...
@pytest.mark.parametrize("test_params", invalid_signup_data.values(), ids=invalid_signup_data.keys())
def test_incorrect_signup(make_user, auth_client, test_params):
    user = make_user(username=test_params.username, password=test_params.password, autogen=False)
    response = auth_client.register_user_request(user=user)

    validate_incorrect_response(response, status=test_params.status_code, message=test_params.expected_message)
